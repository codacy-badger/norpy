...
from norpy.simulate_model import create_state_space, return_immediate_rewards, backward_induction_procedure, simulate
...
def set_up_last_period():
    ...
    periods_emax = backward_induction_procedure(model_object,state_space,immediate_rewards)
    ...
    aux = np.zeros(model_object.num_draws_emax * 3).reshape(model_object.num_draws_emax, 3)
...
def test_last_period_value_func(set_up_last_period):
    ...
    np.testing.assert_array_almost_equal(np.array([set_up_last_period[0]]),
        np.array([set_up_last_period[1][set_up_last_period[2] - 1, set_up_last_period[3]]]),
        decimal=1,
    )
...
def set_up_any_period():
    ...
    k_to_check = np.random.randint(0, state_space["states_number_period"][period_to_check] - 1)
...
def init_simulation(constr=False):
    model_object = get_model_obj(
        get_random_model_specification(constr={'num_types': 4, 'type_spec_shifts': np.zeros(12).reshape(4, 3),}))
    ...
    return dat, model_object.num_periods, model_object.num_agents_sim, period_to_check,model_object.delta, agent_to_check
...
def test_simulation_descriptives(init_simulation):
    ...
    assert init_simulation[0][(init_simulation[5] - 1) * init_simulation[1] + init_simulation[3], 2] in [1, 2, 3]
    assert init_simulation[0][(init_simulation[5] - 1) * init_simulation[1] + init_simulation[3], 17] == init_simulation[4]
...
def init_simulation_huge_rewards():
    model_object = get_model_obj(
    get_random_model_specification(constr={'num_types': 4, 'type_spec_shifts': np.zeros(12).reshape(4, 3),
                                           "coeffs_work":np.concatenate((np.random.uniform(size=10), np.array([2000000000000000, 0, 0]))
    )}))
    dat = simulate(model_object)
    agent_to_check = np.random.randint(0, model_object.num_agents_sim)
    period_to_check = np.random.randint(0, model_object.num_periods - 1)
...
def test_simulation_with_high_work_rewards(init_simulation_huge_rewards):
    assert init_simulation_huge_rewards[0][(init_simulation_huge_rewards[3] - 1) * init_simulation_huge_rewards[1] + init_simulation_huge_rewards[2], 2] == 1
